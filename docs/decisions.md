...
### 2026-05-19 - Resolve PDF phase into first implementation contract
- Decision: PDF phase open questions are resolved into `docs/pdf-ingestion-plan.md` as the first implementation contract. The phase targets a synthetic ICBC-like credit card statement family with `icbc-visa-like-v1` and `icbc-mastercard-like-v1` public synthetic variants. Visa-like implementation can start single-page, while Mastercard-like multi-page support is required before the PDF phase is complete. PdfPig is the first PDF text extraction candidate, QuestPDF is the first synthetic fixture generation candidate, `expectedTotal` remains externally provided, CSV behavior must remain unchanged, and PDF ingestion starts behind an internal application service before a later `POST /api/expense-reports/process-pdf` endpoint.
- Context: MVP v0.1 is complete as a deterministic CSV processing backend, and the previous PDF plan still contained phase-blocking open questions. The project needs a clear PDF contract before implementation so extraction, normalization, privacy, fixture design, and tests do not drift into arbitrary PDF support or private-data handling decisions during coding.
- Alternatives considered: Keep `docs/pdf-open-questions.md` as a separate source of truth, start with arbitrary bank/card PDF support, use anonymized real statements as public fixtures, trust extracted statement totals as validation input, add OCR, use LLM extraction or analysis, introduce runtime application agents, add the public endpoint before extraction behavior is stable, or refactor the CSV pipeline broadly before PDF support requires it.
- Consequences: PDF work can proceed in small, reviewable milestones with synthetic fixtures only, deterministic extraction and normalization, no OCR, no LLM, no runtime application agents, no private data in the repo, and no change to the CSV baseline. Future OCR, LLM work, arbitrary statement support, trusted statement-total extraction, advanced multi-currency processing, real/private statement processing beyond local experimentation, endpoint hardening, and runtime AI architecture require separate decisions.
- Status: Accepted.

### 2026-05-20 - Define internal PDF extraction boundary
- Decision: PDF ingestion will enter the application through `IPdfStatementExtractor`, which accepts a source name, PDF bytes, and an optional statement-shape hint, then returns a normalized extraction result with supported statement shape identifiers, extraction status, normalized rows, invalid extracted rows, warnings, safe evidence, page/order traceability, foreign-currency evidence, and extracted statement totals as metadata only. Real PDF libraries remain isolated from Application and Domain and will be added later in Infrastructure.
- Context: PDF-1 needs a contract that future PDF extraction implementations can use to feed the existing deterministic ExpenseFlow pipeline without replacing `ITransactionFileParser`, changing CSV behavior, adding a public endpoint, or introducing PdfPig/QuestPDF before parser implementation begins.
